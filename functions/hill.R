...
dose_response_profiling <- function(data, time_interval, initial_guess, flag = FALSE) {
  ...
  dose_response_curve <- experiment %>%
    ggplot(data = ., aes(Condition, phi), color = "black")+
  ...
  if (flag == TRUE) {
    ...
    # model[[1]] = dataframe with fitting errors (model_fit)
    # model[[2]] = model details (model)
    
    dose_response_curve <- dose_response_curve +
      geom_line(data = model[[1]], aes(Condition, fi), size = 1.2)
...
maximum_growth_rate <- function(data,
                                time_interval) {
...
hwa_method <- function(data, time_interval, initial_guess, flag = FALSE) {
  
  # Obtain the phi values corresponding to the maximum growth rate
  experiment <- maximum_growth_rate(data = data,
                                    time_interval = time_interval)
