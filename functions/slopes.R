...
hook_plot <- function(data, time_limits) {
...
preliminary_slopes <- function(data, time_interval) { 
  ...
  for (i in seq_along(conditions)) {
    ...
    lm_model <- lm(subset_df$production_rate ~ subset_df$growth_rate)
...
linear_view <- function(data, condition, time_interval, t1 = NaN, t2 = NaN) {
  ...
  # Fit a linear model
  lm_model <- lm(production_rate/1000 ~ growth_rate, data = subset_data)
  ...
  plot_with_time_filter <- subset_data %>%
    ...
    geom_abline(slope = coef(lm_model)[2], intercept = coef(lm_model)[1], color = "#8B0000", size = 1.5) +  # Bigger linewidth
  ...
  plot_without_time_filter <- subset_df %>%
    ...
    xlab(expression(h^-1)) +
    ylab(expression(10^3~FU~OD^-1~h^-1)) +
    geom_point(data = subset_data, aes(x = growth_rate, y = production_rate/1000), color = "#FF6666", size = 3)
...
manual_adjust <- function(input_df, linear_section_matrix, condition, t1 = NaN, t2 = NaN) {
  ...
  if (!is.nan(t1) || !is.nan(t2)) {
    ...
    # Fit a linear model
    lm_model <- lm(production_rate ~ growth_rate, data = subset_df)
