  ...
  tabsetPanel(
               ...
               tabsetPanel(
                 tabPanel("Hill fitting",
                          mainPanel(
                            ...
                            verbatimTextOutput("dose_plot_summary")  # Add this line for text summary
                          )
                 ),
    ...
    tabPanel("Recap",
             sidebarPanel(
               ...
               actionButton("model_prediction_button", "Model Prediction"),
               br(),
               br(),
               # Export Results button
               actionButton("export_results_button", "Export Results")
             ),
             mainPanel(
               # Rendering the hill_plot
               plotOutput("recap_hill_plot")
...
server <- function(input, output, session) {
  ...
  # Render the second element (summary) for the Hill fitting tab
  output$dose_plot_summary <- renderPrint({
    req(file_uploaded())
    # Check if the "Fitting" button is clicked for either method
    if (fitting_button_slope_based() || fitting_button_single_point()) {
      if (input$method_selection == "Single-point method") {
        ...
        # Return the summary of the second element from the list
        summary(dose_response_result[[2]])
      } else if (input$method_selection == "Slope-based method") {
        slope_based_result <- slope_based_visualization(
          linear_df = linear_detection_results_reactive(),
          initial_guess = list(H = input$H, ki = input$ki, n = input$n),
  ...
  # Reactive value for storing the result of hill_prediction
  hill_prediction_result <- reactiveVal(NULL)
  
  # Observe "Model Prediction" button clicks
  observeEvent(input$model_prediction_button, {
    req(file_uploaded())
    
    # Call the hill_prediction function with the specified arguments
    hill_prediction_result(hill_prediction(
      name = uploaded_data(),
      time_limits = list(3, 11),
      initial_guess = list(H = input$H, ki = input$ki, n = input$n),
      growth = seq(0, 0.75, 0.1)
    ))
  })
  
  # Render the ggplot in the main panel of the "Recap" tab
  output$recap_hill_plot <- renderPlot({
    req(hill_prediction_result())
    # Display the ggplot object from the hill_prediction_result
    hill_prediction_result()
  })
}
